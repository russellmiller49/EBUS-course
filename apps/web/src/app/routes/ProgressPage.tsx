...
const progressModuleDefinitions: ProgressModuleDefinition[] = [
  ...
  },
  {
    id: 'case-001',
    title: '3D Anatomy',
    description: 'Tri-planar CT, segmentation, and patient-space anatomy exploration.',
    icon: '◫',
    path: '/cases/case-001',
    routeId: 'case-001',
    moduleIds: ['case-001'],
...
function areStoredModulesComplete(state: LearnerProgressState, moduleIds: ModuleProgressId[] = []) {
  return moduleIds.length > 0 && moduleIds.every((moduleId) => {
...
export function buildProgressModuleCards({
  ...
  return progressModuleDefinitions.map((definition) => {
    ...
    const status: ProgressModuleStatus = completed ? 'completed' : currentStep ? 'current' : unlocked ? 'unlocked' : 'locked';
...
export function ProgressPage() {
  ...
  const moduleCards = useMemo(
    () => buildProgressModuleCards({ accessOptions, state, steps: courseStepModels }),
  ...
  const unlockedModules = moduleCards.filter((module) => module.status === 'current' || module.status === 'unlocked').length;
      ...
      <section className="section-card progress-dashboard">
        <div className="section-card__heading">
          ...
          <div className="tag-row">
            <span className="tag">{completedModules}/{moduleCards.length} modules complete</span>
      ...
      <section className="section-card">
        ...
        <div className="progress-module-grid">
          {moduleCards.map((module) => (
            <article
              className={`progress-module-card progress-module-card--${module.status}`}
              key={module.id}
            >
